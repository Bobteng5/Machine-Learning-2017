...
EVE_LOSS_THRESH_PERCENT = 7.7 / 16
...
RETRAIN_EVE_ITERS = 100  # Retrain eve up to ITERS*LOOPS times.
...
def plot_errors(ac, picture):
    """
    Plot Lowest Decryption Errors achieved by Bob and Eve per epoch
    """
    sns.set_style("darkgrid")
    plt.plot(ac.history_bob_errors)
    plt.plot(ac.history_eve_errors)
    plt.legend(['bob', 'eve'])
    plt.xlabel('Epoch')
    plt.ylabel('Lowest Decryption error achieved (max:{:d})'.format(TEXT_SIZE))
    #plt.show()
    plt.savefig(picture)
...
def train_until_thresh(s, ac):
  ...
  for j in xrange(MAX_TRAINING_LOOPS):
    ...
    #for _ in xrange(ITERS_PER_ACTOR * EVE_MULTIPLIER):
    #  s.run(ac.carol_optimizer)
    if j % (PRINT_EVERY * 40) == 0:
      filepath = save_parameters(s, ac, j)
    bob_avg_loss, eve_avg_loss, carol_avg_loss = doeval(s, ac, EVAL_BATCHES, j)
...
def train_and_evaluate():
  ...
  with tf.Session() as s:
      ...
      for _ in xrange(NUMBER_OF_EVE_RESETS):
        ...
        for _ in xrange(RETRAIN_EVE_LOOPS):
          for _ in xrange(RETRAIN_EVE_ITERS):
            eve_counter += 1
            s.run(ac.eve_optimizer)
          _, tmp, _ = doeval(s, ac, EVAL_BATCHES, eve_counter)
    ...
    plot_errors(ac, options.savemodel + '/nothing.png')
